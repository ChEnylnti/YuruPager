...
export function agentConfirmPrompt(nodeTask) {
    return [
...
export function criteriaCheckPrompt(nodeTask, criteriaText) {
    return [
...
export function parseConditionVerdict(reply) {
    const line = reply
        .split("\n")
        .map((part) => part.trim())
...
export function validateDefinition(definition) {
    if (definition.version !== 1)
        throw new Error(`workflow_definition_version_unsupported: ${String(definition.version)}`);
    if (definition.nodes.length === 0)
        throw new Error("workflow_definition_empty");
    const seen = new Set();
    for (const [index, node] of definition.nodes.entries()) {
        if (typeof node.id !== "string" || node.id.length === 0)
            throw new Error("workflow_node_id_missing");
        if (seen.has(node.id))
            throw new Error(`workflow_node_id_duplicate: ${node.id}`);
        seen.add(node.id);
