...
async function seedAlpha(pool: Pool, password: string): Promise<void> {
  ...
  await pool.query(
    `
      ...
      ON CONFLICT (workspace_id, workstation_id, thread_id) DO UPDATE SET latest_turn_id = EXCLUDED.latest_turn_id, status = EXCLUDED.status, sync_state = EXCLUDED.sync_state, updated_at = EXCLUDED.updated_at;
